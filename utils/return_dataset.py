...
def return_dataset(args):
    ...
    image_set_file_t = os.path.join(
        base_path, "labeled_target_images_" + args.target + "_%d.txt" % (args.num)
    )
    image_set_file_t_val = os.path.join(
        base_path, "validation_target_images_" + args.target + "_3.txt"
    )
    image_set_file_unl = os.path.join(
        base_path, "unlabeled_target_images_" + args.target + "_%d.txt" % (args.num)
    )
    ...
    source_dataset = Imagelists_VISDA(
        image_set_file_s, root=root, transform=data_transforms["train"]
    )
    target_dataset = Imagelists_VISDA(
        image_set_file_t, root=root, transform=data_transforms["val"]
    )
    target_dataset_val = Imagelists_VISDA(
        image_set_file_t_val, root=root, transform=data_transforms["val"]
    )
    target_dataset_unl = Imagelists_VISDA(
        image_set_file_unl, root=root, transform=data_transforms["val"]
    )
    target_dataset_test = Imagelists_VISDA(
        image_set_file_unl, root=root, transform=data_transforms["test"]
    )
...
def return_dataset_test(args):
    ...
    target_dataset_unl = Imagelists_VISDA(
        image_set_file_test, root=root, transform=data_transforms["test"], test=True
    )
    class_list = return_classlist(image_set_file_s)
    print("%d classes in this dataset" % len(class_list))
    if args.net == "alexnet":
